...
def train(net, train_iter, valida_iter, loss,  optimizer, device, epochs=10):    #
    ...
    for epoch in range(epochs):  #epochs=10
        ...
        for X1, X2, y in train_iter:
            ...
            y = y.to(device)
            # print('11111111')
            xout1, xout2, y_hat = net(X1, X2)
            # print('22222222')
            # print('y_hat', y_hat)
            # print('y', y)
            l= loss(y_hat, y.long())
            # print('33333333')
            # l2 = loss2(xout1, xout2, y.long()-1)
            # l = 0.8*l1 + 0.2*l2
            optimizer.zero_grad()
            l.backward()
            optimizer.step()
...
########################################记录每轮epoch###################################################################################
        ...
        # trainAcc_txt = "C:\\Users\倪漂漂\Desktop\ nwhtry.txt"
        # output = "epoch %d, train loss %.6f, train acc %.3f, valida loss %.6f, valida acc %.3f, time %.1f sec" % (
